      ...
      .onSnapshot(snapshot => {
        snapshot.docChanges().forEach(change => {
          ...
          if (change.type === "modified") {
            if (
              change.doc.data().count >= change.doc.data().recruitment_number
            ) {
              console.log(`넘어감`);
  ...
  db.collection("board")
    ...
    .then(querySnapshot => {
      ...
      querySnapshot.forEach(doc => {
        ...
        $title = $(
          `<td id="${doc.id}" class="${doc.data().id}" style="color : blue">${
            doc.data().title
          }</td>`
        );
        $($title).on("click", click_join_title);
        $count = doc.data().count;
        $recruitment = $(`<td>${$count}/${doc.data().recruitment_number}</td>`);
  ...
  db.collection("board")
    ...
    .then(querySnapshot => {
      querySnapshot.forEach(doc => {
        //제목으로 찾기
        if (cate.value == "title") {
          if (`${doc.data().title}`.indexOf(category_input.value) != -1) {
            if (
          ...
          if (`${doc.data().author}`.indexOf(category_input.value) != -1) {
            if (
              `${doc.data().recruitment_number}` ==
              String(recruitment_input.value)
            ) {
